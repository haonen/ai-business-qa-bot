...
from bot.routing_contracts import CommerceScope, MarketScope, MediaScope, RouteDecision, validate_route_decision
...
def _adapt_v2_decision(decision: RouteDecision) -> RouteResult:
    ...
    return RouteResult(
        ...
        message=message, route_decision=decision.to_dict(),
    ...
    if pending.get("intent") == "v2_market_scope":
        ...
        decision = RouteDecision(
            ...
            action=(
                ...
                else "clarify_analysis_scope" if preflight_target
...
def _sanitize_routed_brand(value: str | None) -> str | None:
    ...
    if not candidate:
        return None
    candidate = re.sub(
        r"^\s*(?:请|麻烦|帮我|帮忙|看一下|看看|分析一下|分析|看)\s*",
        "",
        candidate,
        flags=re.IGNORECASE,
    )
    ...
    candidate = re.sub(r"(?:在|于|的)+$", "", candidate).strip()
    candidate = re.sub(r"[，,。！？!?：:=\s]+", "", candidate)
...
def _detect_three_platform_brand(text: str) -> str | None:
    ...
    candidate = re.sub(
        ...
        r"三平台竞品分析|三平台生意报告|三平台生意|天猫|抖音|京东|TMALL|DOUYIN|JINGDONG|"
...
def route(user_text: str, state: SessionState) -> RouteResult:
    ...
    if is_tmall_business_question(text):
        return _route_tmall_business(text, state)
    if state.pending_request and state.pending_request.get("intent") == "business_platform_selection":
        pending = state.pending_request
        if "三平台" in text:
            selected_period = pending_period or pending.get("period")
            return RouteResult(
                type=(
                    "three_platform_competitor_analysis"
                    if selected_period else "clarify_three_platform_period"
                ),
        ...
        selected_platform = _explicit_business_platform(text)
        if selected_platform:
            selected_period = pending_period or pending.get("period")
            route_type = {
                "TM": "default_chain" if selected_period else "clarify_period",
                "DY": "douyin_business_analysis" if selected_period else "clarify_douyin_period",
                "JD": "jd_business_analysis" if selected_period else "clarify_jd_period",
            }[selected_platform]
            return RouteResult(
                type=route_type,
                brand=pending.get("brand"),
