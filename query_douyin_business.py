    ...
    *,
    ...
    for period_key, start_key, end_key in (
        ...
        for item in _source_slices(period_meta[start_key], period_meta[end_key]):
            table = item["table"]
            query_brand = monthly_brand if item["source"] == "monthly" and monthly_brand else brand
            platform_clause = (
                "AND " + platform_filter_sql(S2_MONTHLY_TABLE, "DY")
                if item["source"] == "monthly" else ""
            )
            business_date = (
                store_rank_monthly_date_sql("bus_date")
            ...
            category_column = "category_CN" if item["source"] == "monthly" else "category_level_4"
            if item["source"] == "monthly":
                ...
                sql = f"""
                    ...
                    GROUP BY COALESCE(NULLIF(TRIM(category_CN), ''), '未分类')
                """
            else:
                sql = f"""
                    SELECT
    ...
    *,
    ...
    for period_key, start_key, end_key in (
        ...
        for item in _source_slices(period_meta[start_key], period_meta[end_key]):
            query_brand = monthly_brand if item["source"] == "monthly" and monthly_brand else brand
            if item["source"] == "monthly":
                business_date = store_rank_monthly_date_sql("bus_date")
                sql = f"""
...
def query_douyin_business(
    ...
    try:
        ...
        if sources and len(missing) == len(sources) and any(
            row["source"] == "monthly" for row in sources
        ):
            log.info(
                "[douyin_business] retrying empty monthly coverage brand=%s period=%s",
                source_brand, period,
            )
            monthly_brand = _resolve_s2_monthly_brand(
                brand, source_brand, period_meta, brand_aliases
            )
            if monthly_brand != source_brand:
        ...
        # Daily tables enrich the monthly core.  A timeout or a mapping issue
        # suppresses only that optional module and never discards valid monthly
        # GMV / YoY / category results.
        channel_result: dict = {}
        try:
        ...
        product_result = {
            "series": [], "channels": {}, "series_mapping": {}, "reconciliation": {}
        }
        try:
            product_frame = _query_s4_products(source_brand, period_meta)
            if product_frame.empty:
                limitations.append("商品日表在指定期间无数据，未展示产品系列和商品下钻。")
            else:
                candidate = _product_analysis(
                    product_frame,
                    selected_category=category_result["selected_category"],
                    brand_candidates=[brand, source_brand, *(brand_aliases or [])],
                )
                if category_result["selected_category"] and not candidate["series"]:
                    limitations.append("月表类目与商品日表类目未匹配，未展示产品系列下钻。")
                elif candidate["negative_short_other"]:
                    limitations.append("商品渠道倒减结果出现负数，未展示商品渠道下钻。")
                else:
                    product_result = candidate
        except Exception:
            log.exception("[douyin_business] product enrichment failed")
            limitations.append("商品日表查询超时或连接中断，未展示产品系列和商品下钻。")

        reconciliation = dict(product_result["reconciliation"])
        if channel_result:
        ...
        if product_result["series"] or product_result["channels"]:
            report_sources.append({
                "period": "current+prior",
                "source": "daily",
