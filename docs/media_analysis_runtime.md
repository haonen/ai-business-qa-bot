...
- 指定区间晚于Topline最新月份时，BET各模块统一截到最新可用月；报告顶部同时展示请求区间与实际分析区间，不再仅返回“没有数据”。
- BET报告后只回复“改到5月/到5月也可以”等时间修改时，继承BET品牌和媒体范围，继续生成默认BET分析模板，不切换到GMV追问。
...
运行时品牌解析优先读取`ai_bot_brand_resolution_cache`、
`ai_bot_brand_dictionary`和`ai_bot_tmall_brand_index`，不会用店铺名扫描事实表。
只有用户原始品牌名在目标来源完全查不到时，才读取
`bot/data/brand_cn_en_reference.json`，将无歧义的中文品牌名转换为英文候选并重查。
一个中文名对应多个英文名的冲突项不会自动转换，也不会覆盖原名的直接命中。
