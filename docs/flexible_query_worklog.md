...
- 发布结构：staging 已改为不可变 `releases/<git-revision>` 与原子 `current` 软链接；当前运行 `9778c4a842a1`，保留 `51da73d8fb03` 和第一代 `app`。部署脚本会在切换前校验编译、依赖和运行时导入，启动失败自动恢复旧目标；显式回滚脚本已实测从新版本切回旧版本并恢复最新版，两次服务健康检查均通过。
