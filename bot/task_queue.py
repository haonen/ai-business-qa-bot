...
def claim_recent_key(namespace: str, value: str, ttl_seconds: int, *, redis=None) -> bool:
    redis = redis or get_redis()
    key = f"ai-bot:dedupe:{namespace}:{_digest(value)}"
    return bool(redis.set(key, str(time.time()), nx=True, ex=ttl_seconds))


def release_recent_key(namespace: str, value: str, *, redis=None) -> None:
    (redis or get_redis()).delete(f"ai-bot:dedupe:{namespace}:{_digest(value)}")


def enqueue_request(payload: dict, *, redis=None):
    try:
