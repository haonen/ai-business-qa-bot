    ...
    try:
        ...
        if platform == "TM" and fallback_slices:
            ...
            daily = fetch_df(f"""
                ...
                {daily_from}
                WHERE {store_rank_business_category_sql(category, 'd.category_EN_level_1', 'd.category_EN_level_2')}
                  {daily_segment_clause}
        ...
        for period_key, ranges in (("current", slices), ("prior", prior_slices)):
            for item in ranges:
                ...
                # Only TM has a daily fallback. DY/JD/TTL gaps are missing
                # monthly coverage, rather than missing daily data.
                source = "monthly" if monthly_complete or platform != "TM" else "daily"
