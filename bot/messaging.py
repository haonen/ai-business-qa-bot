...
def stable_uuid(namespace: str, value: str) -> str:
    return str(uuid_lib.uuid5(uuid_lib.NAMESPACE_URL, f"ai-business-qa-bot:{namespace}:{value}"))
...
def send_text(client: lark.Client, chat_id: str, text: str, *, uuid: str | None = None) -> str | None:
    resp = client.im.v1.message.create(
        CreateMessageRequest.builder()
        .receive_id_type("chat_id")
        .request_body(
...
def send_card(client: lark.Client, chat_id: str, card: dict, *, uuid: str | None = None) -> str:
    resp = client.im.v1.message.create(
        CreateMessageRequest.builder()
        .receive_id_type("chat_id")
        .request_body(
    ...
    if not resp.success():
        raise RuntimeError(f"Feishu card send failed: {resp.code} {resp.msg}")
    return resp.data.message_id
