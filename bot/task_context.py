...
def resolve_task_turn(text: str, state: SessionState) -> TaskTurnResolution | None:
    ...
    # An explicit BET-only turn replaces commerce objectives. A semantic
    # MODIFY_SCOPE classification may inherit the brand/date, never add EC work.
    bet_only = bool(current_decision and current_decision.intents == ["BET"])
    goals = [] if bet_only else list(dict.fromkeys([*frame.goals, *frame.intents]))
    semantic_goals = [str(value).upper() for value in semantic.get("goals") or []]
    goals.extend(value for value in semantic_goals if value in GOALS and value not in goals and (not bet_only or value == "BET"))
    if current_decision:
    ...
    intents = list(dict.fromkeys([
        *[value for value in frame.intents if not bet_only and value not in {"BET", "EC_BUSINESS"}],
    ...
    resolved_platform = None if bet_only else explicit_platform_value or frame.platform
    ...
    root_question = text if bet_only else frame.original_question or text
    ...
    return TaskTurnResolution(
        ...
        media_mode=current_decision.media_scope.mode if bet_only else frame.media_mode,
        media_channels=list(current_decision.media_scope.channels if bet_only else frame.media_channels),
