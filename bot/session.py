...
class DomainContext:
    ...
    latest_report_evidence: dict | None = None
...
class BusinessTaskContext:
    ...
    last_slot_patch: list[dict] = field(default_factory=list)


@dataclass
class SessionState:
    brand_lookup_context: dict = field(default_factory=dict)
    ...
    market_lookup_context: dict = field(default_factory=dict)
    category_market_lookup_context: dict = field(default_factory=dict)
...
_TASK_SLOT_NAMES = {
    ...
    "comparison_metric", "rejected_period", "failure_kind",
    "latest_available_date", "awaiting_slot", "status", "last_relation",
}
...
def reduce_task_context(current: BusinessTaskContext, patch: TaskContextPatch) -> BusinessTaskContext:
    ...
    for name, update in patch.slots.items():
        ...
        before = getattr(task, name)
        if op == KEEP:
            slot_trace.append({
                "slot": name, "operation": KEEP, "value": before,
                "source": update.source, "span": update.span, "turn_id": update.turn_id,
            })
            continue
        if op == CLEAR:
        ...
        slot_trace.append({
            "slot": name, "operation": op, "before": before,
            "value": getattr(task, name), "source": update.source,
            "span": update.span, "turn_id": update.turn_id,
        })
    ...
    task.last_slot_patch = slot_trace
    task.turn_count += 1
...
def _state_from_payload(payload: dict) -> SessionState:
    ...
    return SessionState(
        brand_lookup_context=dict(payload.get("brand_lookup_context") or {}),
        ...
        market_lookup_context=dict(payload.get("market_lookup_context") or {}),
        category_market_lookup_context=dict(payload.get("category_market_lookup_context") or {}),
...
def _log_task_transition(
    before: BusinessTaskContext, after: BusinessTaskContext, source: str,
    patch: TaskContextPatch | None = None,
) -> None:
    ...
    fields = (
        ...
        "rejected_period", "failure_kind", "latest_available_date",
        "awaiting_slot", "status",
    )
    before_slots = {name: getattr(before, name) for name in fields}
    after_slots = {name: getattr(after, name) for name in fields}
    explicit_slot_patch = {
        name: {"op": update.op, "value": update.value}
        for name, update in ((patch.slots if patch else {}) or {}).items()
        if isinstance(update, SlotUpdate) and update.op != KEEP
    }
    log.info("[task_context_trace] %s", json.dumps({
        ...
        "explicit_slot_patch": explicit_slot_patch,
        "after_slots": after_slots,
        "task_status": after.status,
        "failure_kind": after.failure_kind,
        "latest_available_date": after.latest_available_date,
...
def apply_task_context_patch(open_id: str, patch: TaskContextPatch) -> BusinessTaskContext:
    ...
    def mutate(state: SessionState):
        ...
        _log_task_transition(before, result, "task_patch", patch)
    _mutate_session(open_id, mutate)
...
def publish_ec_report_context(
    open_id: str, *, evidence: dict, cache: dict, brand: str | None,
    period: str | None, platform: str | None, brand_aliases: list[str] | None = None,
    filters: dict | None = None, source_brands: dict | None = None,
) -> SessionState:
    """Publish task identity and compact report evidence in one session mutation.

    The compact evidence remains available even when the legacy report cache is
    trimmed to satisfy the Redis session size limit.
    """
    def mutate(state: SessionState):
        ctx = state.ec_context
        ctx.brand = brand or ctx.brand
        ctx.period = period or ctx.period
        ctx.platform = platform or ctx.platform
        ctx.brand_aliases = list(brand_aliases or [])
        ctx.filters = dict(filters or {})
        ctx.source_brands = dict(source_brands or {})
        ctx.latest_report_evidence = dict(evidence or {}) or None
        ctx.recent_evidence = [dict(evidence)] if evidence else []
        ctx.report_cache = dict(cache or {})
        state.last_result_cache = dict(cache or {})
        state.task_context = reduce_task_context(state.task_context, TaskContextPatch(
            relation="REPORT_COMPLETED",
            slots={
                "brand": SlotUpdate(SET, brand, source="report_evidence") if brand else SlotUpdate(),
                "period": SlotUpdate(SET, period, source="report_evidence") if period else SlotUpdate(),
                "platform": SlotUpdate(SET, platform, source="report_evidence") if platform else SlotUpdate(),
                "category": SlotUpdate(
                    SET, (filters or {}).get("category"), source="report_evidence",
                ) if (filters or {}).get("category") in {"TTL", "SKIN", "MEX", "HAIR", "MAKEUP"} else SlotUpdate(),
                "status": SlotUpdate(SET, "active", source="report_evidence"),
            },
            reason_codes=["EC_REPORT_EVIDENCE_PUBLISHED"],
        ))
    return _mutate_session(open_id, mutate)
...
def set_brand_lookup_context(open_id: str, context: dict):
    return _mutate_session(open_id, lambda state: setattr(state, "brand_lookup_context", dict(context)))
