...
from bot.messaging import create_lark_client, send_reply, send_text, stable_uuid, update_text
...
def execute_request(payload: dict) -> dict:
    ...
    if not payload.get("placeholder_id"):
        payload["placeholder_id"] = send_text(
            client,
            payload["chat_id"],
            "已收到卡片追问，正在分析数据，请稍候…",
            uuid=stable_uuid("analysis-status", payload["job_id"]),
        )
    ...
    def on_progress(stage_text: str) -> None:
        ...
        event('analysis.progress',event='progress',label=stage_text)
        now = time.monotonic()
        if job is not None:
