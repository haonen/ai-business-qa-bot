...
def build_fraud_result(brand: str, period: str) -> dict:
    """Business QA flagging: item-level ATV > 1000 in 2026 period."""
    ...
    item_df = (
        df26.groupby(["item_id", "kol_driver"], dropna=False)
        .agg(gmv_2026=("gmv", "sum"), unit=("unit", "sum"))
        .reset_index()
    )
    item_df["atv"] = item_df.apply(
        lambda r: float(r["gmv_2026"]) / float(r["unit"]) if r["unit"] else 0,
        axis=1,
    )
