...
ALLOWED_EXECUTORS = {
    ...
    "opportunity_business_report",
    "build_reasoning_pack",
    "synthesize_opportunity_insight",
    "filter_update",
    "synthesize_evidence",
}
...
def agent_plan_enabled() -> bool:
    if os.environ.get("LEGACY_PIPELINE_EMERGENCY", "0").strip().lower() not in {"1", "true", "yes", "on"}:
        return True
...
def compile_agent_plan(route_result: Any, user_text: str) -> AgentPlan:
    ...
    if route_type not in NO_DATA_ROUTES and route_type != "data_availability":
        from bot.dynamic_plan import compile_dynamic_plan, dynamic_plan_enabled
        from bot.execution_policy import use_standard_execution
        if dynamic_plan_enabled() and not use_standard_execution(route_result):
            return compile_dynamic_plan(route_result, user_text)
        if use_standard_execution(route_result) and route_type == "skill_dispatch":
            # The existing followup chain selects EC/BET, validates scope and
            # reuses matching evidence. Do not force BET into the EC-only DAG.
            plan = AgentPlan(
                _plan_id(route_type, route_id, user_text), route_type,
                "复用报告证据并补充缺失数据", "followup", route_type, True, False,
                (PlanStep("execute", "回答新增问题", "followup_tool_bundle", inputs=common),),
                (), route_id, response_strategy="TARGETED_ANSWER",
            )
    ...
    if route_type == "opportunity_analysis":
        from bot.semantic_analysis import build_semantic_analysis_request
        semantic_request = build_semantic_analysis_request(route_result, user_text).to_dict()
    ...
    elif route_type == "opportunity_analysis":
        opportunity_inputs = {
            **common,
            "semantic_request": semantic_request,
            "selected_skills": list((semantic_request or {}).get("selected_skills") or []),
        }
        steps = (
            PlanStep(
                "evidence", "读取或生成当前范围的生意证据", "opportunity_business_report",
                inputs=opportunity_inputs, kind="TEMPLATE",
                capability_id="opportunity.business_report", output_schema="evidence-envelope.v3",
            ),
            PlanStep(
                "reasoning_pack", "组织跨维度机会与风险证据", "build_reasoning_pack",
                ("evidence",), opportunity_inputs, kind="DERIVE",
                capability_id="derive.opportunity_reasoning_pack", output_schema="reasoning-pack.v3",
            ),
            PlanStep(
                "synthesize", "结合原问题、数据和Skills形成行动假设", "synthesize_opportunity_insight",
                ("evidence", "reasoning_pack"), opportunity_inputs, kind="SYNTHESIS",
                capability_id="synthesis.opportunity_insight", output_schema="opportunity-insight.v3",
            ),
        )
        plan = AgentPlan(
            _plan_id(route_type, route_id, user_text), route_type, "生意表现、增长机会与行动方案",
            "semantic-planner-v3", route_type, True, True, steps,
            tuple((semantic_request or {}).get("requested_dimensions") or dimensions), route_id,
            "agent-reasoning-v3", "MULTI_STEP_ANALYSIS", semantic_request,
        )
...
def validate_agent_plan(plan: AgentPlan) -> None:
    max_steps = max(1, int(os.environ.get("AGENT_PLAN_MAX_STEPS", "12")))
    ...
    for step in plan.steps:
        ...
        if step.kind == "DERIVE":
            ...
            if step.capability_id == "derive.opportunity_reasoning_pack":
                continue
        ...
        if step.foreach and int(step.foreach.get("max_items") or 0) > 10:
            raise PlanValidationError(f"step {step.step_id} foreach max exceeds 10")
...
def completion_status(route_type: str | None, meta: dict | None = None) -> str:
    failure_kind = (meta or {}).get("failure_kind")
    if failure_kind in {"DATA_AFTER_LATEST", "NO_DATA_IN_RANGE"}:
        return "这个时间范围的数据暂不齐全："
    if failure_kind or (meta or {}).get("error_code"):
        return "本次分析未完成："
    if not smart_progress_enabled():
        return "已完成，结果如下："
